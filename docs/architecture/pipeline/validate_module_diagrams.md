...
```mermaid
flowchart TB
    subgraph Current["当前分支 codex/content-enum-resolution · 第一阶段及 2A 已实现"]
        ...
        Scope --> Resolver["ContentArgumentResolver · 本次新增<br/>成员、别名、参数默认值、赋值快照<br/>同名遮蔽、循环检测"]
        Resolver --> Shared["common/content_contracts.py · 本次迁移<br/>三个枚举类与成员查询<br/>原 content_vocab 导入路径重导出同一类"]
        Resolver --> Legacy["compat/content_syntax.py<br/>旧文本准入规则集中保留"]
        Shared --> Result["ContentEnumResolution<br/>实际值、预期枚举族、来源、状态"]
        Legacy --> Result
        Result --> Check["Binding / Validate / Typecheck · 本次接通<br/>非法成员与循环：semantic<br/>错误枚举族与非文本：typecheck<br/>词表、配对、surface / cells 约束继续检查"]
        ...
        Guard -->|是| Stop["2A 执行保护 · 本次新增<br/>PLAN_CONTENT_ENUM_EXECUTION_UNSUPPORTED<br/>不产生可执行计划，防止写入空分类"]
        Guard -->|否| Runtime["现有 Runtime<br/>旧协议执行、registry 与 JSON 保持兼容"]
        Check --> Deferred["未确定的参数仍 deferred<br/>最终绑定值检查属于 2C"]
    end

    subgraph Next["下一步 2B · 待实现"]
        Classification["ContentClassification<br/>持有 ContentKind / ContentType<br/>统一入口验证配对后创建"]
        PairContract["共享层承接配对契约<br/>与旧输入及历史转换结果分开"]
        PairContract --> Classification
    end
    Check -.-> PairContract
    Classification -.-> Boundary["后续 2C · 待实现<br/>绑定后、物料更新前复核<br/>Plan / Runtime 保留枚举身份<br/>接通后移除 2A 执行保护"]
    ...
    Legend["图例<br/>绿色：已实现；本次改动在节点中标注<br/>橙色虚线：下一步 2B<br/>紫色虚线：后续 2C–2D<br/>灰色：当前限制 / 更晚事项"]
    ...
    class Classification,PairContract next;
    class Boundary,Conformance later;
...
sequenceDiagram
    ...
    rect rgb(220, 252, 231)
        Note over Author, Plan: 当前 2A 已实现：公开函数均有测试覆盖
        ...
        opt 仅旧输入需要转换
            ...
            Legacy-->>Check: canonical 字符串、原分类元数据与警告
        end
    ...
    rect rgb(255, 237, 213)
        Note over Check, Shared: 下一步 2B 待实现
        Check->>Shared: 统一入口验证 kind / type 配对
        Shared-->>Check: ContentClassification（实际枚举成员）
    end
    rect rgb(237, 233, 254)
        Note over Check, Runtime: 后续 2C–2D 待实现
        Check->>Plan: 2C 枚举身份进入计划与绑定
        Plan->>Shared: 2C 最终绑定值复核
        Shared-->>Plan: 有效分类 / 明确诊断
        Plan->>Runtime: 2C 接通后解除执行保护
        Runtime->>Shared: 物料更新前复核
        Note over Legacy, Runtime: 2D：旧协议计算、JSON、回放和 reference 对照<br/>attrs.role 保持开放，code / name 保持字符串
    end
```
...
classDiagram
    ...
    ContentVocabulary ..> SharedContentContracts : 同一枚举类重导出
    ...
    ConstructorValidator ..> ContentVocabulary : 当前配对表
    ...
    SharedContentContracts ..> ContentClassification : 下一步：验证配对后创建
    ...
    note for ContentArgumentResolver "本次 2A 已实现 · pipeline/content_inputs.py<br/>成员、别名、默认值、遮蔽、循环与赋值快照<br/>test_content_inputs.py + test_content_enum_resolution.py"
    note for SharedContentContracts "本次迁入枚举定义，旧 Python 导入保持类身份<br/>配对表迁移及强类型分类对象属于下一步 2B"
    note for ContentEnumExecutionGuard "本次新增：阻止空分类写入<br/>PLAN_CONTENT_ENUM_EXECUTION_UNSUPPORTED<br/>2C 执行端接通后移除"
    note for LegacyContentSyntax "旧源码规则集中保留<br/>大版本移除时与历史数据适配分开"
    note for ContentClassification "图例：绿色已实现；橙色下一步 2B<br/>紫色后续 2C–2D；灰色执行限制"
    ...
    style ContentClassification fill:#ffedd5,stroke:#c2410c,color:#7c2d12,stroke-dasharray:5 5
    style ContentBoundaryValidator fill:#ede9fe,stroke:#7c3aed,color:#4c1d95,stroke-dasharray:5 5
